...
- 모든 코드는 SOC2 컴플라이언스 기준을 준수해야 한다. 상세 내용은 `SOC2.md` 참조.
...
- 모듈 간 의존성은 명시적이어야 하며, 순환 참조(circular dependency)를 만들지 않는다.
...
### React / Next
- 반드시 yarn을 사용한다
- css는 Tailwind CSS v4를 사용한다.
- 컴포넌트 라이브러리는 shadcn/ui를 기반으로 하며, Radix UI primitive를 사용한다.
- cn() 유틸리티 (clsx + tailwind-merge)를 사용하여 클래스를 조합한다.
...
### NestJS
- DBMS는 <MySQL or PostgreSQL>를 사용하며, ORM은 Sequelize를 사용한다.
- 캐시는 Redis를 사용한다.
...
- 모듈 내 컨트롤러가 3개 이상인 경우, `controllers/` 디렉토리를 생성하여 컨트롤러 파일들을 분리한다.
  - 예: `src/modules/account/controllers/account.controller.ts`
  - 모듈 파일에서 import 경로를 `./controllers/xxx.controller`로 수정한다.

### FastAPI
- DBMS는 <MySQL or PostgreSQL>를 사용하며, ORM은 Tortoise를 사용한다.
- 캐시는 Redis를 사용한다.
- 서버 프레임워크는 FastAPI + Python + UV를 사용한다.
- 반드시 test 코드를 작성하고, 이를 통해 테스트하여야 한다.
...
- 불확실한 요구사항이 있는 경우, 추측으로 구현하지 말고 **명확한 가정을 문서화**하거나 질문한다.
