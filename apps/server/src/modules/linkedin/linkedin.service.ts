...
export class LinkedinService {
    ...
    preview(file?: Express.Multer.File): LinkedinPreview {
        ...
        return {
            ...
            educations: this.readCsv(zip, 'Education.csv').map((row) => ({
                ...
                degree: [this.pick(row, ['Degree Name', 'Degree']), this.pick(row, ['Field Of Study', 'Field'])].filter(Boolean).join(', ') || null,
                startDate: this.toDate(this.pick(row, ['Started On', 'Start Date'])),
                endDate: this.toDate(this.pick(row, ['Finished On', 'End Date'])),
                description: this.pick(row, ['Notes', 'Activities']),
            })),
            skills: this.readCsv(zip, 'Skills.csv').map((row) => this.pick(row, ['Name', 'Skill'])).filter((value): value is string => value !== null),
        };
    }

    async import(dto: LinkedinImportDto): Promise<{ experiencesImported: number; educationsImported: number }> {
        ...
        for (const position of dto.positions ?? []) {
            const exists = await this.experienceModel.findOne({
                where: { company: position.company, role: position.role, startDate: position.startDate },
            });

            if (!exists) {
                await this.experienceModel.create({ ...position, sortOrder: position.sortOrder ?? 0, visible: position.visible ?? true });
        ...
        for (const education of dto.educations ?? []) {
            const exists = await this.educationModel.findOne({
                where: { institution: education.institution, degree: education.degree, startDate: education.startDate },
            });

            if (!exists) {
                await this.educationModel.create({ ...education, sortOrder: education.sortOrder ?? 0, visible: education.visible ?? true });
    ...
    private readCsv(zip: AdmZip, fileName: string): CsvRow[] {
        const entry = zip.getEntries().find((candidate) => candidate.entryName.endsWith(fileName));
    ...
    private parseProfile(rows: CsvRow[]): Record<string, string | null> {
        ...
        return {
            ...
            publicProfileUrl: this.pick(row, ['Public Profile Url', 'Public Profile URL']),
        };
    }
