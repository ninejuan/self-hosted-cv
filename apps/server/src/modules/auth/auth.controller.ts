...
export class AuthController {
    ...
    @Post('2fa/verify')
    verifyTwoFactor(@Body() dto: TotpCodeDto, @Req() request: Request): Promise<{ enabled: true }> {
        return this.authService.verifyTwoFactor(dto, request);
    }

    @Post('2fa/disable')
    disableTwoFactor(@Body() dto: TotpCodeDto, @Req() request: Request): Promise<{ enabled: false }> {
