...
export class ContactService {
    ...
    findAll(): Promise<SocialLink[]> {
        return this.socialLinkModel.findAll({ order: [['sortOrder', 'ASC'], ['createdAt', 'ASC']] });
    }

    create(dto: CreateContactDto): Promise<SocialLink> {
        return this.socialLinkModel.create({ ...dto, sortOrder: dto.sortOrder ?? 0, visible: dto.visible ?? true });
    }
    ...
    async reorder(dto: ReorderDto): Promise<SocialLink[]> {
        await this.sequelize.transaction(async (transaction) => {
            await Promise.all(dto.items.map((item) => this.socialLinkModel.update({ sortOrder: item.sortOrder }, { where: { id: item.id }, transaction })));
        });
