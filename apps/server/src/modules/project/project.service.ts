...
export class ProjectService {
    ...
    findAll(): Promise<SideProject[]> {
        return this.projectModel.findAll({ order: [['sortOrder', 'ASC'], ['date', 'DESC']] });
    }

    create(dto: CreateProjectDto): Promise<SideProject> {
        return this.projectModel.create({ ...dto, sortOrder: dto.sortOrder ?? 0, visible: dto.visible ?? true });
    }
    ...
    async reorder(dto: ReorderDto): Promise<SideProject[]> {
        await this.sequelize.transaction(async (transaction) => {
            await Promise.all(dto.items.map((item) => this.projectModel.update({ sortOrder: item.sortOrder }, { where: { id: item.id }, transaction })));
        });
