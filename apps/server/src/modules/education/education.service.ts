...
export class EducationService {
    ...
    findAll(): Promise<Education[]> {
        return this.educationModel.findAll({ order: [['sortOrder', 'ASC'], ['startDate', 'DESC']] });
    }

    create(dto: CreateEducationDto): Promise<Education> {
        return this.educationModel.create({ ...dto, sortOrder: dto.sortOrder ?? 0, visible: dto.visible ?? true });
    }
    ...
    async reorder(dto: ReorderDto): Promise<Education[]> {
        await this.sequelize.transaction(async (transaction) => {
            await Promise.all(dto.items.map((item) => this.educationModel.update({ sortOrder: item.sortOrder }, { where: { id: item.id }, transaction })));
        });
