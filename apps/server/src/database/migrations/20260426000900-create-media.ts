...
export async function up({ context }: { context: QueryInterface }): Promise<void> {
    await context.createTable('media', {
        ...
        status: { type: DataTypes.ENUM('pending', 'uploaded', 'attached', 'deleted'), allowNull: false, defaultValue: 'pending' },
        created_at: { type: DataTypes.DATE, allowNull: false, defaultValue: Sequelize.literal('CURRENT_TIMESTAMP') },
        updated_at: { type: DataTypes.DATE, allowNull: false, defaultValue: Sequelize.literal('CURRENT_TIMESTAMP') },
    });
    await context.addIndex('media', ['entity_type', 'entity_id', 'sort_order'], { name: 'media_entity_type_entity_id_sort_order_idx' });
    await context.addIndex('media', ['storage_key'], { name: 'media_storage_key_idx' });
