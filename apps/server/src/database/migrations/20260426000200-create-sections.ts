...
export async function up({ context }: { context: QueryInterface }): Promise<void> {
    await context.createTable('sections', {
        ...
        type: { type: DataTypes.ENUM('work_experience', 'writing', 'speaking', 'side_project', 'education', 'contact'), allowNull: false },
    ...
    await context.addIndex('sections', ['profile_id', 'sort_order'], { name: 'sections_profile_id_sort_order_idx' });
    await context.addIndex('sections', ['profile_id', 'type'], { name: 'sections_profile_id_type_idx' });
}
