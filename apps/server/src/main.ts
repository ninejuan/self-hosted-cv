...
async function bootstrap() {
    ...
    app.use(helmet({
        contentSecurityPolicy: {
            directives: {
                ...
                frameAncestors: ["'none'"],
                objectSrc: ["'none'"],
                scriptSrc: ["'self'", (request) => `'nonce-${cspNonces.get(request) ?? ''}'`],
    ...
    app.use((request: Request, response: Response, next: NextFunction) => {
        ...
        const isLogin = request.method === 'POST' && path.startsWith('/api/auth/login');
        const isCsrfToken = request.method === 'GET' && path.startsWith('/api/auth/csrf-token');
